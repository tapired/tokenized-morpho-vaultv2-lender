...
interface IMorphoVaultV2Like is IERC4626 {
    /// @notice Returns whether an account can send assets into the vault.
    function canSendAssets(address account) external view returns (bool);

    /// @notice Returns whether an account can receive shares from the vault.
    function canReceiveShares(address account) external view returns (bool);

    /// @notice Returns whether an account can send shares out of the vault.
    function canSendShares(address account) external view returns (bool);

    /// @notice Returns whether an account can receive assets from the vault.
...
contract MorphoVaultV2Limits {
    ...
    /// @param strategy_ The strategy querying the limit.
    ...
    function availableDepositLimit(
        ...
        if (
            !morphoVault.canSendAssets(strategy_) ||
            !morphoVault.canReceiveShares(strategy_)
        ) {
            return 0;
        }
    ...
    function vaultsMaxWithdraw(
        ...
        IMorphoVaultV2Like morphoVault = IMorphoVaultV2Like(morphoVault_);
        if (
            !morphoVault.canSendShares(strategy_) ||
            !morphoVault.canReceiveAssets(strategy_)
        ) {
            return 0;
        }
